...
def main(prediction,bbx):
    ...
    video_1 = np.array(get_video(args.video_name))[np.newaxis]
    video_2 = copy.deepcopy(video_1)
    video_3 = copy.deepcopy(video_1)

    videos = np.concatenate([video_1, video_2, video_3], 0)
    ...
    for frame_id in range(seq_len):
        if first_frame:
            try:
                ...
                init_rect = np.array([[149, 132, 42, 51],
                                      [149, 132, 44, 53],
                                      [149, 132, 46, 55]])
            except:
                exit()
    ...
    for v_id in range(frame.shape[0]):
        imageio.mimsave('./demo/output_%s.gif' % v_id, save_results[v_id], 'GIF', duration = 0.5)
