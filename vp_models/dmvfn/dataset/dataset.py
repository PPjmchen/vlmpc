...
class LanguageTableTrainDataset(Dataset):
    def __init__(self, batch_size=16):
        ...
        if not os.path.exists('./data/shuffle_all'):
            ...
            self.data_root = './data/shuffle_all'
            file_list = os.listdir(self.data_root)
            self.gif_files = [os.path.join(self.data_root, filename) for filename in file_list if filename.endswith('.gif')]
            self.action_files = [os.path.join(self.data_root, filename) for filename in file_list if filename.endswith('.pth')]
    ...
    def __getitem__(self, idx):
        ...
        frames = torch.from_numpy(np.array(imageio.mimread(self.gif_files[idx]))).permute(0, 3, 1, 2)
        actions = torch.from_numpy(torch.load(self.action_files[idx]))

        if frames.shape[0] < 9:
            frames = torch.cat([frames[0].unsqueeze(0).repeat([9-frames.shape[0], 1, 1, 1]), frames], 0)
        if actions.shape[0] < 9:
            actions = torch.cat([actions[0].unsqueeze(0).repeat([9-actions.shape[0], 1]), actions], 0)
...
if __name__ == '__main__':
    ...
    for i in range(len(language_table_train_dataset)):
        language_table_train_dataset[i]
