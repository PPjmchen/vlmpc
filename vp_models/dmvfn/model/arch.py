...
class MVFB(nn.Module):
    ...
    def forward(self, x, flow, scale):
        ...
        if x2.shape[2] != x4.shape[2]:
            x2 = F.interpolate(x2, size=(x4.shape[2], x4.shape[3]), mode="bilinear", align_corners=False)
        else:
...
class DMVFNAct(nn.Module):
    def __init__(self, action_dim=2):
        super(DMVFNAct, self).__init__()
        
        self.block0 = MVFB(13+4+2*action_dim, num_feature=160)
        self.block1 = MVFB(13+4+2*action_dim, num_feature=160)
        self.block2 = MVFB(13+4+2*action_dim, num_feature=160)
        self.block3 = MVFB(13+4+2*action_dim, num_feature=80)
        self.block4 = MVFB(13+4+2*action_dim, num_feature=80)
        self.block5 = MVFB(13+4+2*action_dim, num_feature=80)
        self.block6 = MVFB(13+4+2*action_dim, num_feature=44)
        self.block7 = MVFB(13+4+2*action_dim, num_feature=44)
        self.block8 = MVFB(13+4+2*action_dim, num_feature=44)
    ...
    def forward(self, x, scale, actions, training=True):
        ...
        if training:
            for i in range(9):
                flow_d, mask_d = stu[i](torch.cat((img0, img1, warped_img0, warped_img1, action0, action1, mask), 1), flow,
                                        scale=scale[i])
            ...
            for i in range(9):
                if ref[0, i]:
                    flow_d, mask_d = stu[i](torch.cat((img0, img1, warped_img0, warped_img1, action0, action1, mask), 1), flow, scale=scale[i])
