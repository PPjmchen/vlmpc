...
def evaluate(model, args):
    ...
    for i in range(10):
        with torch.no_grad():
            ...
            save_img = np.array(pred.cpu().squeeze() * 255).transpose(1, 2, 0) # CHW -> HWC
            cv2.imwrite(args.output_dir, save_img)

            img_0 = img_1
            img_1 = pred.squeeze().cpu().numpy()

            all_frames.append(save_img[..., ::-1].astype('uint8'))

    imageio.mimsave('./images/output.gif', all_frames, 'GIF', duration = 0.5)
